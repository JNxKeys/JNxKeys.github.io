...
Las páginas de producto activas (`windows/`, `eset/`, `google one/`, `kaspersky/`, `office/`, `chatgpt/`) siguen la misma estructura, replicada manualmente en cada archivo (no hay componentes compartidos):
...
- **Sección "¿Qué incluye?"** (`.feat-section`, misma clase reutilizada en `windows/` y `chatgpt/`): lista de funciones/beneficios concretos del producto, con ícono + nombre + descripción corta por ítem. Windows además tiene una segunda sección "Beneficios generales" que compara Windows 10 vs 11 Pro (10 ya no recibe soporte de Microsoft desde oct. 2025).
- **Nota de venta cruzada hacia combos** (`.combo-note`, en `windows/` y `chatgpt/`): enlaza al combo de `/combos/` que incluye ese producto. Los combos, a su vez, enlazan de vuelta a la página del producto individual dentro de su listado "incluye" — el enlace es bidireccional.
...
El hub (`index.html`) agrupa los productos en categorías (`.group`/`.eyebrow`): Sistema Operativo (Windows), Productividad (Office), Inteligencia Artificial (Google One AI Plus, ChatGPT Plus — agrupados juntos porque su propuesta central es IA, no ofimática), Seguridad (ESET, Kaspersky).
...
- Métodos de pago por página: Windows, ESET, Office y ChatGPT Plus aceptan Yape (Perú) y Western Union (internacional, enruta a la misma cuenta Yape). Google One y Kaspersky solo aceptan Yape — la activación/suscripción de esos dos está atada técnicamente a cuentas de Perú, así que no tiene sentido ofrecer pago internacional ahí.
- **Apple One está desactivado temporalmente** (a pedido del negocio): no aparece en el hub ni en el sitemap, y tiene `meta robots noindex`. La página sigue en el repo intacta para reactivarla fácilmente — para reactivarla hay que devolver su tarjeta al hub, agregarla de nuevo al sitemap.xml y quitar el noindex.
- Productos que existen en la Base de Conocimiento pero **todavía no tienen página propia**: ESET Small Business Security (venta B2B por volumen, no es un simple card de precio fijo). Canva Pro y CapCut Pro se mencionaron como posibles productos futuros pero aún no tienen precio/modalidad definidos — hay que pedirle esos datos al negocio antes de crearles página.
