...
Las páginas de producto activas (`windows/`, `eset/`, `google one/`, `kaspersky/`, `office/`) siguen la misma estructura, replicada manualmente en cada archivo (no hay componentes compartidos):
...
- Métodos de pago por página: Windows, ESET y Office aceptan Yape (Perú) y Western Union (internacional, enruta a la misma cuenta Yape). Google One y Kaspersky solo aceptan Yape — la activación/suscripción de esos dos está atada técnicamente a cuentas de Perú, así que no tiene sentido ofrecer pago internacional ahí.
...
- Productos que existen en la Base de Conocimiento pero **todavía no tienen página propia**: ChatGPT Plus (S/50/mes, ya aparece como componente de un combo) y ESET Small Business Security (venta B2B por volumen, no es un simple card de precio fijo). Canva Pro y CapCut Pro se mencionaron como posibles productos futuros pero aún no tienen precio/modalidad definidos.
